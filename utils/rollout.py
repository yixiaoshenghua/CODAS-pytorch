from configs.config import ImgShape
...
class MappingHolder(object):
    """
    NOTE: you should create a new MappingHolder instance for each episode.  # CHECK
    """
    def __init__(self, stack_imgs, is_dynamics_emb, var_seq, default_rnn_state, obs_shape,
        ...
        self.cur_state = default_rnn_state.clone()
...
class Runner(object):
    ...
    def run_traj(self, deter, mapping_holder, render_img, run_in_realworld):
        ...
        dones_traj = np.zeros((self.max_horizon), dtype=np.int)
        ...
        ob = select_env.reset()
        # if do_mapping:
            # mapping_holder.set_first_state(init_obs=ob)    # CHECK: why commenting this line?
        total_rew = 0
        i = 0
        ...
        return {self.TOTAL_REW: total_rew, self.TRAJ_LEN: i,
                ...
                self.R2S_OB_TRAJ: de_normalize_r2s_ob_traj, self.IMG_TRAJ: img_traj, self.DONE_TRAJ: dones_traj}
