...
class TrajectoryBuffer(ReplayBuffer):
    def __init__(self, size, pop=False, has_img=False):
        super(TrajectoryBuffer, self).__init__(size)
        self.has_img = has_img
        self.pop = pop

    def _encode_sample(self, batch_size):
        encode_datas = [[] for _ in range(self.tuple_size)]
        for _ in range(batch_size):
            i = random.randint(0, len(self._storage) - 1)
            if self.pop:
                data = self._storage.pop(i)
            else:
                data = self._storage[i]
            for i in range(self.tuple_size):
                encode_datas[i].append(np.array(data[i], copy=False))

        for i in range(self.tuple_size):
            encode_datas[i] = np.array(encode_datas[i])
        return encode_datas
